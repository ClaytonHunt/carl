...
preserve_user_data() {
    ...
    # Note: index.carl is deprecated - modern CARL uses structured .carl/project/ hierarchy
}
...
restore_user_data() {
    ...
    # Note: index.carl restoration removed - modern CARL uses structured .carl/project/ hierarchy
}
...
apply_version_migrations() {
    ...
    case "$current_version" in
        ...
        "1.3.3"|"1.3.2"|"1.3.1")
            echo "   ⬆️ Migrating to v1.4.0+ personality system..."
            migrate_personality_system "$target_dir"
            echo "   ⬆️ Migrating to v1.4.1+ architecture overhaul..."
...
migrate_strategic_context_integration() {
    ...
    # Note: index.carl strategic files migration removed - strategic files now managed in .carl/project/ hierarchy
}

# Migrate to v1.4.1+ architecture overhaul
migrate_architecture_overhaul() {
    local target_dir="$1"
    ...
    mkdir -p "$target_dir/.carl/sessions/archive"
    mkdir -p "$target_dir/.claude/agents"
    
    # Migrate old template files to new naming convention
    local old_templates=("context.template" "intent.template" "state.template" "mission.template" "roadmap.template" "decisions.template")
    for old_template in "${old_templates[@]}"; do
        local old_path="$target_dir/.carl/templates/$old_template"
        local new_name="${old_template%.template}.template.carl"
        local new_path="$target_dir/.carl/templates/$new_name"
        
        if [ -f "$old_path" ] && [ ! -f "$new_path" ]; then
            mv "$old_path" "$new_path" 2>/dev/null || true
            echo "         ✅ Migrated $old_template to $new_name"
        fi
    done
    
    # New template files should now be in place from copy_carl_files
    # Just verify they exist and report status
    local new_templates=("vision.template.carl" "objectives.template.carl" "process.template.carl" "index.template.carl")
    for template in "${new_templates[@]}"; do
        local target_template="$target_dir/.carl/templates/$template"
        if [ -f "$target_template" ]; then
            echo "         ✅ Verified new template $template"
        else
            echo "         ⚠️ Missing new template $template"
        fi
    done
    
    # New format specifications should now be in place from copy_carl_files
    # Just verify they exist and report status
    local spec_files=("context.format.carl" "intent.format.carl" "state.format.carl" "process.format.carl" "vision.format.carl" "roadmap.format.carl" "objectives.format.carl" "migration.strategy.carl")
    for spec in "${spec_files[@]}"; do
        local target_spec="$target_dir/.carl/specs/$spec"
        if [ -f "$target_spec" ]; then
            echo "         ✅ Verified specification $spec"
        else
            echo "         ⚠️ Missing specification $spec"
        fi
    done
    ...
    for agent in "${new_agents[@]}"; do
        ...
        if [ -f "$target_agent" ]; then
            echo "         ✅ Verified specialist agent $agent"
        else
            echo "         ⚠️ Missing specialist agent $agent"
        fi
    done
    
    # Verify session manager exists (should be copied by copy_carl_files)
    local session_manager="$target_dir/.carl/scripts/carl-session-manager.sh"
    if [ -f "$session_manager" ]; then
        chmod +x "$session_manager"
        echo "         ✅ Verified session manager script"
    else
        echo "         ⚠️ Missing session manager script"
    fi
    
    # Migrate personalities.carl to personalities.config.carl if needed
    local old_personalities="$target_dir/.carl/personalities.carl"
    local new_personalities="$target_dir/.carl/personalities.config.carl"
    if [ -f "$old_personalities" ] && [ ! -f "$new_personalities" ]; then
        mv "$old_personalities" "$new_personalities" 2>/dev/null || true
        echo "         ✅ Migrated personalities to new naming convention"
    fi
...
create_example_files() {
    ...
    # Note: index.carl creation removed - modern CARL uses structured .carl/project/ hierarchy
    echo "   ℹ️  Modern CARL uses .carl/project/ structure instead of index.carl"
...
# Note: Project type detection now uses .carl/project/ structure instead of index.carl
EOF
...
show_installation_summary() {
    ...
    echo -e "${PURPLE}🎯 Project Type: ${NC}$(echo "detected during analyze" || echo "unknown")"
    ...
    echo "   - .carl/project/ (Project context for AI)"
    echo ""
...
main() {
    ...
    if [ "$UPDATE_MODE" = true ]; then
        ...
        # Apply migrations based on version difference (after files are available)
